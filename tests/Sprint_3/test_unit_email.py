...
from app import db, email, Candidato, obtener_correos_aptos, obtener_correos_noaptos, enviar_correos
...
def test_obtener_correos_aptos(app, setup_db):
    with app.app_context():
        candidatosAptos = obtener_correos_aptos()
...
def test_obtener_correos_no_aptos(app, setup_db):
    with app.app_context():
        candidatoNoAptos = obtener_correos_noaptos()
        ...
        with patch.object(email, 'connect', autospec=True) as mock_connect:
            ...
            for msg in llamadas:
                ...
                if destinatario in aptos:
                    assert "Hola, hemos revisado tu curriculum y estamos interesados en tu perfil." in msg.body
...
def test_envio_email_no_apto(app, setup_db):
    with app.app_context():
        with patch.object(email, 'connect', autospec=True) as mock_connect:
            ...
            no_aptos = obtener_correos_noaptos()
            ...
            for msg in llamadas:
                ...
                if destinatario in no_aptos:
                    assert "Hola, lamentamos que en esta oportunidad tu perfil no se ajusta a lo que buscamos." in msg.body